...
import { ApiService, RegistrarDireccionEntregaRequest } from 'src/app/Services/api.services';
import { AuthService } from 'src/app/features/auth/services/auth.service';
import { GlobalVariable } from 'src/app/VarGlobals';
import { noWhitespaceValidator } from 'src/app/shared/validators/form-validators';

@Component({
  selector: 'app-direccion-entrega-register-dialog',
  templateUrl: './direccion-entrega-register-dialog.component.html',
  styleUrls: ['./direccion-entrega-dialog.component.scss']
})
export class DireccionEntregaRegisterDialogComponent {
  readonly form: FormGroup;
  isSaving = false;
  errorMessage = '';

  constructor(
    private readonly dialogRef: MatDialogRef<DireccionEntregaRegisterDialogComponent>,
    private readonly formBuilder: FormBuilder,
    private readonly apiService: ApiService,
    private readonly authService: AuthService
  ) {
    this.form = this.formBuilder.group({
      dirDes: ['', [Validators.required, noWhitespaceValidator(), Validators.maxLength(250)]],
      dirUbi: ['', [Validators.required, noWhitespaceValidator(), Validators.maxLength(250)]]
    });
  }
  ...
  guardar(): void {
    ...
    const payload: RegistrarDireccionEntregaRequest = {
    ...
    this.apiService.registrarDireccionEntrega(payload).subscribe({
      next: () => {
        this.isSaving = false;
        this.dialogRef.close(true);
      },
      error: (error: unknown) => {
        console.error('Error registrando direccion de entrega:', error);
        this.errorMessage = this.getErrorMessage(error, 'registrar');
        this.isSaving = false;
      }
    });
  }
