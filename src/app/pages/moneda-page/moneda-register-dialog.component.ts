...
import { ApiService, RegistrarMonedaRequest } from 'src/app/Services/api.services';
import { AuthService } from 'src/app/features/auth/services/auth.service';
import { GlobalVariable } from 'src/app/VarGlobals';
import { noWhitespaceValidator } from 'src/app/shared/validators/form-validators';

@Component({
  selector: 'app-moneda-register-dialog',
  templateUrl: './moneda-register-dialog.component.html',
  styleUrls: ['./moneda-dialog.component.scss']
})
export class MonedaRegisterDialogComponent {
  readonly form: FormGroup;
  isSaving = false;
  errorMessage = '';

  constructor(
    private readonly dialogRef: MatDialogRef<MonedaRegisterDialogComponent>,
    private readonly formBuilder: FormBuilder,
    private readonly apiService: ApiService,
    private readonly authService: AuthService
  ) {
    this.form = this.formBuilder.group({
      monDes: ['', [Validators.required, noWhitespaceValidator(), Validators.maxLength(120)]],
      monAbr: ['', [Validators.required, noWhitespaceValidator(), Validators.maxLength(15)]]
    });
  }
  ...
  guardar(): void {
    ...
    const payload: RegistrarMonedaRequest = {
    ...
    this.apiService.registrarMoneda(payload).subscribe({
      next: () => {
        this.isSaving = false;
        this.dialogRef.close(true);
      },
      error: (error: unknown) => {
        console.error('Error registrando moneda:', error);
  ...
  private getErrorMessage(error: unknown): string {
    if (!(error instanceof HttpErrorResponse)) {
      return 'No se pudo registrar la moneda. Intenta nuevamente.';
    }
    ...
    return `No se pudo registrar la moneda. Codigo HTTP: ${error.status}.`;
  }
