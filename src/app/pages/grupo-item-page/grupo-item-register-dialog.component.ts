...
@Component({
  ...
  templateUrl: './grupo-item-register-dialog.component.html',
  styleUrls: ['./grupo-item-dialog.component.scss']
})
export class GrupoItemRegisterDialogComponent {
  readonly form: FormGroup;
  isSaving = false;
  errorMessage = '';

  constructor(
    private readonly dialogRef: MatDialogRef<GrupoItemRegisterDialogComponent>,
  ...
  guardar(): void {
    ...
    const payload: RegistrarGrupoItemRequest = {
      Grp_Des: String(this.form.controls['grpDes'].value || '').trim(),
      Usr_Reg: this.getCurrentOperator()
    };

    this.apiService.registrarGrupoItem(payload).subscribe({
      next: () => {
        this.isSaving = false;
        this.dialogRef.close(true);
      },
      error: (error: unknown) => {
        console.error('Error registrando grupo de item:', error);
  ...
  private getErrorMessage(error: unknown): string {
    if (!(error instanceof HttpErrorResponse)) {
      return 'No se pudo registrar el grupo de item. Intenta nuevamente.';
    }
    ...
    return `No se pudo registrar el grupo de item. Codigo HTTP: ${error.status}.`;
  }
