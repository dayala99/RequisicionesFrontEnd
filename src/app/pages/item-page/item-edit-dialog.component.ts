...
interface ItemEditData { item: { itmId:number; itmDes:string; itmGrp:number; itmSubGrp?:number; itmDetMatId?:number; flgEst:string; }; }
...
export class ItemEditDialogComponent {
  ...
  grupos: Option[]=[]; subGrupos: Option[]=[]; detallesMaterial: Option[]=[];
  isLoadingGroups=true; isLoadingSubGroups=false; isLoadingDetalles=false; isSaving=false; errorMessage='';
  constructor(@Inject(MAT_DIALOG_DATA) public data:ItemEditData, private ref:MatDialogRef<ItemEditDialogComponent>, fb:FormBuilder, private api:ApiService, private auth:AuthService){
    this.form=fb.group({itmDes:[data.item.itmDes,[Validators.required,noWhitespaceValidator(),Validators.maxLength(120)]],itmGrp:[data.item.itmGrp,Validators.required],itmSubGrp:[data.item.itmSubGrp??null,Validators.required],itmDetMat:[data.item.itmDetMatId??null,Validators.required],flgEst:[data.item.flgEst||'A',Validators.required]});
    this.cargarGrupos();
  }
