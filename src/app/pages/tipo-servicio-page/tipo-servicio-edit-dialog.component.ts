...
@Component({
  selector: 'app-tipo-servicio-edit-dialog',
  templateUrl: './tipo-servicio-edit-dialog.component.html',
  styleUrls: ['./tipo-servicio-dialog.component.scss']
})
export class TipoServicioEditDialogComponent {
  readonly form: FormGroup;
  isSaving = false;
  errorMessage = '';

  constructor(
    @Inject(MAT_DIALOG_DATA) public readonly data: TipoServicioEditData,
    private readonly dialogRef: MatDialogRef<TipoServicioEditDialogComponent>,
  ...
  guardar(): void {
    ...
    const payload: ActualizarTipoServicioRequest = {
      Tip_Ser_Id: this.data.tipoServicio.tipSerId,
      Tip_Ser_Des: String(this.form.controls['tipSerDes'].value || '').trim(),
      Flg_Est: String(this.form.controls['flgEst'].value || 'A'),
      Usr_Mod: this.getCurrentOperator()
    };

    this.apiService.actualizarTipoServicio(payload).subscribe({
      next: () => {
        this.isSaving = false;
  ...
  private getErrorMessage(error: unknown): string {
    if (!(error instanceof HttpErrorResponse)) {
      return 'No se pudo actualizar el tipo de servicio. Intenta nuevamente.';
    }
    ...
    return `No se pudo actualizar el tipo de servicio. Codigo HTTP: ${error.status}.`;
  }
