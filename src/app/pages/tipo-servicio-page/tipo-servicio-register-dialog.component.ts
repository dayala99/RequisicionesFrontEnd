...
@Component({
  ...
  templateUrl: './tipo-servicio-register-dialog.component.html',
  styleUrls: ['./tipo-servicio-dialog.component.scss']
})
export class TipoServicioRegisterDialogComponent {
  readonly form: FormGroup;
  isSaving = false;
  errorMessage = '';

  constructor(
    private readonly dialogRef: MatDialogRef<TipoServicioRegisterDialogComponent>,
  ...
  guardar(): void {
    ...
    const payload: RegistrarTipoServicioRequest = {
      Tip_Ser_Des: String(this.form.controls['tipSerDes'].value || '').trim(),
      Usr_Reg: this.getCurrentOperator()
    };

    this.apiService.registrarTipoServicio(payload).subscribe({
      next: () => {
        this.isSaving = false;
        this.dialogRef.close(true);
      },
      error: (error: unknown) => {
        console.error('Error registrando tipo de servicio:', error);
  ...
  private getErrorMessage(error: unknown): string {
    if (!(error instanceof HttpErrorResponse)) {
      return 'No se pudo registrar el tipo de servicio. Intenta nuevamente.';
    }
    ...
    return `No se pudo registrar el tipo de servicio. Codigo HTTP: ${error.status}.`;
  }
