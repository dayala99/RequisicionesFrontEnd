...
import { ApiService, RegistrarCentroCostoRequest } from 'src/app/Services/api.services';
import { AuthService } from 'src/app/features/auth/services/auth.service';
import { GlobalVariable } from 'src/app/VarGlobals';
import { noWhitespaceValidator } from 'src/app/shared/validators/form-validators';

@Component({
  selector: 'app-centro-costo-register-dialog',
  templateUrl: './centro-costo-register-dialog.component.html',
  styleUrls: ['./centro-costo-dialog.component.scss']
})
export class CentroCostoRegisterDialogComponent {
  readonly form: FormGroup;
  isSaving = false;
  errorMessage = '';

  constructor(
    private readonly dialogRef: MatDialogRef<CentroCostoRegisterDialogComponent>,
    private readonly formBuilder: FormBuilder,
    private readonly apiService: ApiService,
    private readonly authService: AuthService
  ) {
    this.form = this.formBuilder.group({
      cenCosDes: ['', [Validators.required, noWhitespaceValidator(), Validators.maxLength(120)]]
    });
  }
  ...
  guardar(): void {
    ...
    const payload: RegistrarCentroCostoRequest = {
      Cen_Cos_Des: String(this.form.controls['cenCosDes'].value || '').trim(),
      Usr_Reg: this.getCurrentOperator()
    };

    this.apiService.registrarCentroCosto(payload).subscribe({
      next: () => {
        this.isSaving = false;
        this.dialogRef.close(true);
      },
      error: (error: unknown) => {
        console.error('Error registrando centro de costo:', error);
  ...
  private getErrorMessage(error: unknown): string {
    if (!(error instanceof HttpErrorResponse)) {
      return 'No se pudo registrar el centro de costo. Intenta nuevamente.';
    }
    ...
    return `No se pudo registrar el centro de costo. Codigo HTTP: ${error.status}.`;
  }
